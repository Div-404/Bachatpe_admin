...
@Component({
  ...
  styleUrl: './balance-withdraw.component.scss'
})
export class BalanceWithdrawComponent {
  ...
  ngOnInit(): void {
    ...
    this.withdrawForm = this.fb.group({
      Amount: ['', [Validators.required, Validators.min(1), this.maxBalanceValidator()]],
      Comment: ['']
    })

    this.dateChange()
    this.getDw(1, 10)
    this.shared.setSidebrActiveClass('dwmain/credit')
  }
  ...
  getDw(Initial: any, MaxCount: any) {
    ...
    this.api.getBalanceWithdraw(data).subscribe({
      next: (res: any) => {
        this.shared.loader(false)
        if (res && res.lstTrans) {
          this.dwList = (res.lstTrans || []).sort((a: any, b: any) => {
          ...
          this.dwList = (res || []).sort((a: any, b: any) => {
            const tA = a.oReq?.oTime?.Tm_Str || ''
            const tB = b.oReq?.oTime?.Tm_Str || ''
            return tB.localeCompare(tA)
          })
          this.Count = res.length
          this.withdrawRequests = res
        } else {
          this.dwList = []
          this.Count = 0
        }
        this.pager = this.pagination.getPager(this.Count, this.page, this.numRecord)
        const start = (this.page - 1) * this.numRecord
        this.pagedItems = this.dwList.slice(start, start + this.numRecord)
      }, error: (err: any) => {
        this.shared.loader(false)
  ...
  openWithdrawModal(content3: any) {
  ...
  getWithdrawRequests() {
    const obj = { Field1: this.userId }
    this.api.getBalanceWithdraw(obj).subscribe({
      next: (res: any) => {
        if (res && res.lstTrans) {
          this.withdrawRequests = res.lstTrans
        } else if (Array.isArray(res)) {
          this.withdrawRequests = res
        } else {
      ...
      error: () => this.toster.error("Failed to load withdraw requests", "Error")
    })
  }
  ...
  makeWithdraw() {
    ...
    const payload = {
      ...
      Profile: Number(this.userId),
      Bal_CB: 1
    }

    this.api.makeWalletWithdraw(payload).subscribe({
      next: (res: any) => {
        if (res.Result === true) {
          const wdPayload = {
          ...
          this.api.withdrawRequestWithoutToken(wdPayload).subscribe({
            next: (res2: any) => {
              this.shared.loader(false)
              this.closeMd()
              this.page = 1
              this.getDw(1, 10)
              this.toster.success("Withdrawal successful", "Success")
            },
            error: () => {
              this.shared.loader(false)
              this.closeMd()
              this.page = 1
              this.getDw(1, 10)
              this.toster.success("Withdrawal processed", "Success")
            }
          })
        } else {
          this.shared.loader(false)
          this.toster.error(res.MSG_USER || "Withdrawal failed", "Error")
        }
      },
      error: () => {
        this.shared.loader(false)
  ...
  updateWithdrawReqStatus(reqId: number, status: any, selEl: HTMLSelectElement) {
    Swal.fire({
      title: 'Confirmation',
      ...
      if (result.isConfirmed) {
        this.confirmUpdateWithdrawReqStatus(reqId, status, selEl)
      } else {
        selEl.value = '1'
      }
    })
  ...
  private confirmUpdateWithdrawReqStatus(reqId: number, status: any, selEl: HTMLSelectElement) {
  ...
  setPage(page: number) {
    ...
    this.page = page
    this.pager = this.pagination.getPager(this.Count, this.page, this.numRecord)
    const start = (page - 1) * this.numRecord
    this.pagedItems = this.dwList.slice(start, start + this.numRecord)
  }

  private maxBalanceValidator(): import('@angular/forms').ValidatorFn {
    return (control: import('@angular/forms').AbstractControl) => {
      const val = Number(control.value)
      if (val > 0 && val > this.balance) return { maxBalance: true }
      return null
    }
  }
