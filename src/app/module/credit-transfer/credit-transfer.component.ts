...
@Component({
  ...
  standalone: true,
  imports: [NgbDropdownModule, CommonModule, FormsModule, ReactiveFormsModule, NgSelectModule, NgbTooltipModule],
  templateUrl: './credit-transfer.component.html',
  styleUrl: './credit-transfer.component.scss'
})
export class CreditTransferComponent implements OnInit {
  ...
  MAKE_CREDIT_TRANS_DEPT() {
    ...
    this.api.MAKE_CREDIT_TRANS_DEPT(obj).subscribe((data: any) => {
      ...
      if (data.Result === true) {
        ...
        this.closeDeposit()
        this.GET_ADM_CREDIT_TRANS(1, 100)
      } else {
        this.closeDeposit()
  ...
  getUserBalance(): void {
    ...
    const tag = 'balance';
    let obj = {
      profileId: this.selectedUserProfileID,
      balance: 'BALANCE'
    }
