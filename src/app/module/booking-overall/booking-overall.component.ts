...
@Component({
  selector: 'app-booking-overall',
  standalone: true,
  imports: [CommonModule, FormsModule, NgbTooltipModule, CommonmoduleModule],
  templateUrl: './booking-overall.component.html',
  styleUrl: './booking-overall.component.scss'
})
export class BookingOverallComponent implements OnInit{
    ...
    ngOnInit(): void {
      ...
      this.route.params.subscribe(params => {
        ...
        if (this.ServiceId) {
          ...
          this.idRoute = true
          this.shared.setSidebrActiveClass('booking-overall/' + this.ServiceId)
        ...
        //  this.getRecntRechBookUtiMis(1, 10)
          ...
          this.idRoute = false
          this.ServiceId= '0'
          this.shared.setSidebrActiveClass('booking-overall')
          this.clearFiltersOnly()
          this.dateChange()
           this.selectTab('tab1')
          // this.getRecntRechBookUtiMis(1, 10);
  ...
  getSummList() {
    ...
    let data = {
      "Key": "",
      "Field1": "3"   //servicetype   MT=1,Recharge=2,booking=3,utility=4,miscellaneous=5
    }
    ...
    getRecntRechBookUtiMis(Initial: any, MaxCount: any) {
      ...
      this.api.getRecntRechBookUtiMis(data).subscribe({
        next: (res: any) => {
          this.shared.loader(false)
          this.bookingList = res.lstTrans
          this.Count = res.Count
          this.pager = this.pagination.getPager(this.Count, this.page, this.numRecord);
          console.log("here is res from money transfer", this.bookingList);
    ...
    getRecBookUtiMisDetail(Initial: any, MaxCount: any) {
      ...
      this.api.getRecBookUtiMisDetail(data).subscribe({
        next: (res: any) => {
          ...
          console.log('here is summ list', this.bookingList);
  ...
  onChangePage(pageOfItems: Array<any>) {
    ...
    setPage(page: number) {
      ...
      if ((page >= 1) && (page <= this.pager.totalPages)) {
        ...
        this.pagedItems = this.bookingList.slice(((this.pageRecord * page) - this.pageRecord + 1), this.pageRecord * page);
