...
@Component({
  selector: 'app-recharge-overall',
  standalone: true,
  imports: [CommonModule, FormsModule, CommonmoduleModule, NgbTooltipModule],
  templateUrl: './recharge-overall.component.html',
  styleUrl: './recharge-overall.component.scss'
})
export class RechargeOverallComponent implements OnInit{
     ...
     this.route.params.subscribe(params => {
       ...
       if (this.ServiceId) {
        this.shared.activeName$.subscribe((activeName: string) => {
         ...
         this.idRoute = true
         this.shared.setSidebrActiveClass('recharge-overall/' + this.ServiceId)
  ...
  getSummList() {
    ...
    let data = {
      "Key": "",
      "Field1": "2"   //servicetype   MT=1,Recharge=2,booking=3,utility=4,miscellaneous=5
    }
   ...
   getRecntRechBookUtiMis(Initial: any, MaxCount: any) {
    ...
    this.shared.loader(true)
     ...
     this.api.getRecntRechBookUtiMis(data).subscribe({
       next: (res: any) => {
         this.shared.loader(false)
         this.rechargeList = res.lstTrans
         this.Count = res.Count
         this.pager = this.pagination.getPager(this.Count, this.page, this.numRecord);
         console.log("here is res from money transfer", this.rechargeList);
    ...
    this.detailApi= true
     ...
     this.api.getRecBookUtiMisDetail(data).subscribe({
       next: (res: any) => {
         ...
         console.log('here is summ list', this.rechargeList);
  ...
  onChangePage(pageOfItems: Array<any>) {
   ...
   setPage(page: number) {
     ...
     if ((page >= 1) && (page <= this.pager.totalPages)) {
       ...
       this.pagedItems = this.rechargeList.slice(((this.pageRecord * page) - this.pageRecord + 1), this.pageRecord * page);
