...
@Component({
  selector: 'app-money-transfer-overall',
  standalone: true,
  imports: [CommonModule, NgbTooltipModule, NgSelectModule, FormsModule, CommonmoduleModule],
  templateUrl: './money-transfer-overall.component.html',
  styleUrl: './money-transfer-overall.component.scss'
})
export class MoneyTransferOverallComponent implements OnInit {
...
this.getSource()
    this.route.params.subscribe(params => {
      ...
      // this.ServiceId = String(params['id'] || '0');
      // console.log('ServiceId:', typeof this.ServiceId, this.ServiceId);
      if (this.ServiceId) {
        ...
        this.idRoute = true
        this.shared.setSidebrActiveClass('money-transfer-overall/' + this.ServiceId)
        ...
        this.idRoute= false
          this.shared.setSidebrActiveClass('money-main/money-detail')
        this.clearFiltersOnly()
        this.dateChange()
        this.ServiceId= '0'
        // this.shared.setSidebrActiveClass('money-transfer-overall')
       
        this.getMoneyRecent(1, 10)
      }
    });
  ...
  Count: any = 0
  getMoneyRecent(Initial: any, MaxCount: any) {
    this.pageOfItems= []
    ...
    this.api.getRecentMoney(data).subscribe({
      next: (res: any) => {
        this.shared.loader(false)
        this.moneyTransList = res.lstTrans
  ...
  summDetailList: any
  getMoneyDeatail(Initial: any, MaxCount: any) {
    this.moneyTransList= []
    this.pageOfItems= []
    this.detailApi = true
    this.Count= 0
    ...
    this.shared.loader(true)
    this.api.getMoneyDetail(data).subscribe({
      next: (res: any) => {
        this.shared.loader(false)
        ...
        this.moneyTransList = res.lstTrans
        // this.page=1
        console.log('here is summ list', this.moneyTransList);
...
clearFiltersOnly()
{
  ...
  setPage(page: number) {
    ...
    if ((page >= 1) && (page <= this.pager.totalPages)) {
      ...
      this.pagedItems = this.moneyTransList.slice(((this.pageRecord * page) - this.pageRecord + 1), this.pageRecord * page);
      //  this.getLedger(((this.pageRecord*page)-this.pageRecord+1), this.pageRecord*page)
      if (!this.detailApi) {
        this.getMoneyRecent(((this.pageRecord * page) - this.pageRecord + 1), this.pageRecord * page,)
      } else {
        this.getMoneyDeatail(((this.pageRecord * page) - this.pageRecord + 1), this.pageRecord * page,)
      }
